...
class TestCudanet(object):
    ...
    def test_mat_vec(self):
        ...
        o = np.zeros_like(a)
        d_a = self.be.array(a.copy())
        d_b = self.be.array(b.copy())
        d_c = self.be.array(c.copy())

        d_t = self.be.array(np.zeros_like(a))
    ...
    def test_topkcost(self):
        ...
        dprobs = self.be.array(probs)
        dlabels = self.be.array(labels)
    ...
    def test_stack(self):
        ...
        d_vmats = []
        for i in range(len(rdims)):
            d_vmats.append(self.be.array(vmats[i]))

        d_hmats = []
        for i in range(len(cdims)):
            d_hmats.append(self.be.array(hmats[i]))
    ...
    def test_T_field(self):
        ...
        assert np.max(np.abs(gm.numpy_array - c)) < 10**-2, "Error in CUDAMatrix.dot with Transposedarray exceeded threshold"
    ...
    def test_get_row_slice(self):
        ...
        da = self.be.array(ha)
        dslice = self.be.array(np.zeros(((end-start, n)),
        ...
        dprod = self.be.array(np.zeros(hprod.shape, dtype=np.float32))
    ...
    def test_mlp(self):
        ...
        ddata = self.be.array(hdata)
        dlabels = self.be.array(hlabels)
        dweights1 = self.be.array(hweights1.copy())
        dweights2 = self.be.array(hweights2.copy())

        for epoch in range(nepochs):
            herror = 0.0
            derror = self.be.array(np.zeros((1,1), dtype=np.float32))
    ...
    def test_maxpool(self):
        ...
        Tkz = self.be.array(result.reshape((N,C*P*Q)).transpose().copy())
        Ikz = self.be.array(data.reshape((N,C*H*W)).transpose().copy())
        Gkz = self.be.array(grads.reshape((N,C*P*Q)).transpose().copy())
        Bkz = self.be.array(backgrads.reshape((N,C*H*W)).transpose().copy())
    ...
    def test_adadelta(self):
        ...
        d_eGradSq = self.be.array(np.zeros_like(deltX))
        d_eDeltSq = self.be.array(np.zeros_like(deltX))
        d_deltX =   self.be.array(np.zeros_like(deltX))

        for i in range(Ni):
            d_grads = self.be.array(grads[i,:,:])
    ...
    def test_convolution_slice(self):
        ...
        Tkz = self.be.array(np.zeros_like(C2))
        Wkz = self.be.array(filters.reshape((K,C*R*S)).transpose().copy())
        Ikz = self.be.array(data.reshape((N,C*H*W)).transpose().copy())
        # Wkz = self.be.array(filters.reshape((K,C*R*S))).T
        # Ikz = self.be.array(data.reshape((N,C*H*W))).T
        ...
        Ekz = self.be.array(senderror.transpose().copy()) # Same as senderror
        ...
        data.shape = (N, C*H*W)
        Ikz = self.be.array(data.transpose().copy())
    ...
    def test_divide(self):
        print('\divide:')
        A1 = np.float32(np.random.randn(10,10))
        A2 = np.float32(np.random.randn(10,10))
        B1 = self.be.array(A1)
        B2 = self.be.array(A2)
        
        C = B1/B2
        C.copy_to_host()
        np.testing.assert_array_almost_equal(C.numpy_array, A1/A2, 5, "Divide not equal to numpy divide")
    ...
    def test_add(self):
        ...
        d_d = d_a +d_c        
        d_d.copy_to_host()
        errorv = np.linalg.norm(d - d_d.numpy_array)
        assert errorv < 10**-5, "Error in sum exceeded threshold"
        
        a += c
        d_a += d_c
        errorv = np.linalg.norm(a - d_a.data)
        assert errorv < 10**-5, "Error in sum exceeded threshold"
    ...
    @attr('slice')
    def slice_test(self):
        A = np.float32(np.random.rand(17,17))
        B = self.be.array(A)
        
        for i in range(1000):
            rowstart, rowstop, colstart, colstop = np.random.randint(0,17,4)
            if rowstart > rowstop: continue
            if colstart > colstop: continue
            #print rowstart, rowstop, colstart,colstop
            
            np.testing.assert_equal(B[rowstart:rowstop,colstart:colstop].data,A[rowstart:rowstop,colstart:colstop], "slice not equal to numpy array")
    ...
    def test_mean(self):
        ...
        m = self.be.array(a)
        mt1 = self.be.array(t1)
        mt2 = self.be.array(t2)
    ...
    def test_crossmap_response_norm(self):
        ...
        d_inputs = self.be.array(inputs.copy())
        d_inputs2 = self.be.array(inputs.copy()+self.eps)
        d_outputs = self.be.array(np.array(np.empty((C*H*W,N)), dtype=np.single, order='C'))
        d_outputs2 = self.be.array(np.array(np.empty((C*H*W,N)), dtype=np.single, order='C'))
        d_error = self.be.array(error.copy())
        d_berror = self.be.array(np.array(np.empty((C*H*W,N)), dtype=np.single, order='C'))
    ...
    def test_local_contrast_norm(self):
        ...
        d_inputs = self.be.array(inputs.copy())
        d_outputs = self.be.array(np.array(np.empty((C*H*W,N)), dtype=np.single, order='C'))
        d_md = self.be.array(np.array(np.empty((C*H*W,N)), dtype=np.single, order='C'))
        d_dn = self.be.array(np.array(np.empty((C*H*W,N)), dtype=np.single, order='C'))
        d_error = self.be.array(error.copy())
        d_berror = self.be.array(np.array(np.empty((C*H*W,N)), dtype=np.single, order='C'))
    ...
    def test_unpool(self):
		...
		d_inputs = self.be.array(inputs.copy())
		d_rinputs = self.be.array(np.zeros_like(inputs))
		d_outputs = self.be.array(np.zeros_like(outputs))
		...
		d_outputs = self.be.array(outputs.copy())
    ...
    def test_avg_pool(self):
        ...
        d_outputs = self.be.array(np.zeros_like(outputs))
        d_error = self.be.array(error.copy())
        d_berror = self.be.array(np.zeros_like(berror))
    ...
    def test_clip(self):
        ...
        a = np.array(np.random.randn(m, k)*10, dtype=np.float32, order='C')
        m1 = self.be.array(a)
        m2 = self.be.array(np.zeros_like(a))
    ...
    def test_softmax(self):
        ...
        np.random.seed(0)
        a = np.array(np.random.randn(m, n)*10, dtype=np.float32, order='C')
        da = self.be.array(a.copy())
        dc2 = self.be.array(np.zeros_like(a))
    ...
    def test_crossent(self):
        ...
        dProbs = self.be.array(probs.copy())
        dLbls = self.be.array(lbls.copy())
    ...
    def test_minmax(self):
        ...
        for op in 'min', 'max', 'argmax', 'argmin':
            ...
            for sign in (1, -1):
                ...
                da = self.be.array(a)
                dr10 = self.be.array(t0)
                dr11 = self.be.array(t1)
